...
export default function DetailsScreen ({route, navigation}) {
    ...
    Alert.alert('ARE YOU SURE?', 'If you delete this message, it will not be recoverable.', [
        {text: 'Confirm', onPress:()=>{console.log('Confirm Pressed'); dispatch(removeMessage(messages[route.params.detailID].id)); navigation.navigate('History');}},
                ...
                <Text style={{ fontSize: 25, fontWeight: 'bold', marginBottom: 20, color: 'black'}}>Here are the details of this Cipher:</Text>
                    <View>
                        <Text style={{marginBottom: 25}}>
                            Your original message was: {messages[route.params.detailID].message}<br/>
                            The Cipher key was: {messages[route.params.detailID].shift}<br/>
                            Your Cipher type: {messages[route.params.detailID].type}<br/>
